...
if [ -d $dynamorio_path ]; then
    ...
    echo -e "#!/bin/bash\n$drrun_path -c $script_dir/build/libheaptracer.so -- \$@" > $script_dir/heaptracer.sh
    chmod 777 $script_dir/heaptracer.sh
    echo "$0: Done. Run \"heaptracer.sh <application>\" to start tracing."
